...
for i in range(lookback_limit + 1):
    ...
    c_high = df_m['High'].iloc[idx].item() if isinstance(df_m['High'].iloc[idx], pd.Series) else df_m['High'].iloc[idx]
    ...
    if len(df_m[:idx]) >= base_period:
        ...
        # Check conditions
        if c_close >= base_low_val and c_close > c_open and c_close > df_m['Close'].iloc[idx-1]:
            ...
            # Condition D (Engulfing / Pullback)
            ...
            cond_d = is_pullback and (c_close > c_open * 1.15) and (c_close > prev_body_top)
            
            if cond_a or cond_b or cond_c or cond_d:
                print(f"==> PATTERN FOUND at idx {idx}! A={cond_a}, B={cond_b}, C={cond_c}, D={cond_d}")
                print(f"Details: close={c_close:.2f}, open={c_open:.2f}, vol={c_vol}, avg_vol={base_avg_vol_val}")
        
        print(f"Idx {idx}: Open={c_open:.2f}, Close={c_close:.2f}, High={c_high:.2f}, Vol={c_vol}")
